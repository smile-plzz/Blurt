...
**2026-08-13 build (Orchestrator MVP + Reminder scheduling scaffold):** `api/orchestrator.js` splits the single moment/urgency/framing call into explicit persona-read + reminder-plan steps and adds a `schedule_hint`/`decomposition_candidate` signal; `reminder/scheduler.js` turns item 10's design answer (repair-vs-silent-recovery switch) into tested code for the first time, consuming that hint. This still does not move item 8, 9, or 10 to "answered" — the numbers involved (follow-up delays, the repair-count threshold, window-to-hour mapping) are explicitly documented in `reminder/README.md` and `reminder/scheduler.js` as untuned starting guesses. What changed is that there's now a concrete, swappable mechanism to point real-use data at, not a stronger claim about what the right values are.
