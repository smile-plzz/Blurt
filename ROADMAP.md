...
| Frontend | 🟡 in progress — 14-screen mockup set done (`frontend/mockups/`, "organic" design system, plus the dedicated ten-question onboarding spec); home feed, `direct`/`inquiring`/`not sure` check-ins, task decomposition, both rollup views, **Onboarding, Recovery Mode entry, and Settings all now built** (`frontend/web/`), reskinned to match the mockups' actual design system, screen-by-screen verified in Chrome. Remaining gap: Capture's own screens (4-5) still use their original prototype styling, not this design system. |
...
**Next logical build step:** real-use test step 4's automatic moment/urgency/framing decisions against the step 2/3 manual baseline (the API call is confirmed live) — does the model's framing choice and "why" line actually feel better calibrated than picking it by hand? Also: Capture's own screens (mockup screens 4-5) haven't been reskinned to the shared design system yet, and the onboarding "Edit" flow in Settings currently restarts the ten-question flow from scratch rather than resuming with prior answers pre-filled.
