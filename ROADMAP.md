...
| Persona & Inference | 🟡 in progress — step 4 MVP scaffolded (`api/infer.js` + `frontend/web/app.js`'s `inferDecision()`): one Mistral call returns moment/urgency/receptivity/framing, falls back to steps 2-3 on failure. Confirmed live and working (`MISTRAL_API_KEY` set in Vercel, verified via direct API call) — still needs real-use judgment-quality testing, not just plumbing. Onboarding now feeds it a real ten-field persona (`schema/persona.json` 0.2.0) instead of nothing. |
