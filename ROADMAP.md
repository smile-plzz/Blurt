...
**Next logical build step:** real-use test step 4's automatic moment/urgency/framing decisions against the step 2/3 manual baseline (the API call is confirmed live) — does the model's framing choice and "why" line actually feel better calibrated than picking it by hand? Also: the onboarding "Edit" flow in Settings currently restarts the ten-question flow from scratch rather than resuming with prior answers pre-filled; and `capture/web/`'s service worker cache-first strategy needs a version bump reminder written down somewhere so stale-asset debugging (hit once this session) doesn't recur silently for the next person testing it locally.
