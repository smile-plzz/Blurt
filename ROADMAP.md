...
| Frontend | 🟢 all mockup-designed screens built and on-design-system, verified against a refined "Final mockup set" pass (2026-08-11) — tag/state color vocabulary remapped, floating framing-tag chip removed from check-ins, FAB recentered, "Closed this week" feed grouping, decomposition switched to round check-icons, onboarding reworked to select-then-Next/Skip, Capture gained tap-anywhere-to-stop + live transcript preview. **New: Task detail screen** (`frontend/web/task.html`/`task.js`) — groups a decomposed parent's subtasks by has-a-date/no-date/closed and implements the deadline-confirmation prompt (decomposition decision #8, previously listed as not built). Supersedes the old in-modal per-parent rollup. |
| Persona & Inference | 🟡 in progress — step 4 MVP scaffolded (`api/infer.js` + `frontend/web/app.js`'s `inferDecision()`): one Mistral call returns moment/urgency/receptivity/framing, falls back to steps 2-3 on failure. **2026-08-11: onboarding persona is now actually wired into the decision** — was captured at onboarding and never read again until this fix; `inferDecision()` sends `primary_stall_point`/`avoidance_driver`/`energy_windows`/`preferred_framing`/`drop_prone_domains` as priors, and `api/infer.js`'s system prompt translates each into the behavior its onboarding question documented (dread never escalates urgency, "rarely" energy caps urgency and prefers `inquiring`, a self-care intent is never framed like a deadline, etc.). Verified live: identical intent, different persona → different urgency/receptivity/framing. Still needs real-use judgment-quality testing beyond this synthetic verification. |
| Reminder & Tone | 🟡 scheduling scaffolded (2026-08-13, `reminder/scheduler.js`) — fixed-interval timing + the already-decided repair-vs-silent-recovery follow-up logic as pure functions, not yet wired to any live trigger (no push/notification channel exists). Tone/copy validation against real ADHD users is still fully open and is the part this build can't complete by itself. |
| Orchestrator | 🟡 MVP built (2026-08-13, `api/orchestrator.js`) — two-step persona-read → reminder-plan pipeline per `orchestrator/REVIEW.md`, composed inside one Vercel endpoint. `frontend/web/app.js`'s `inferDecision()` now calls it instead of `/api/infer` (kept as a fallback). Adds `schedule_hint` (feeds the new Reminder scaffold) and `decomposition_candidate` as advisory signals. |
| Privacy | ✅ decided for the self-test phase (`PRIVACY.md`, 2026-08-12/13) — local-vs-cloud call made, voice-data STT gap documented with a migration path, opt-out tiers and retention/deletion designed (implementation pending schema sign-off), scope-creep review done against every other agent's actual output. Still owned continuously, not closed permanently — revisit triggers listed in `PRIVACY.md` §1. |
...
5. ✅ Recovery Mode — built (`frontend/web/recovery.html`/`recovery.js`), reached automatically from the home feed's entry gate (`app.js`'s `maybeEnterRecovery()`) when a stalled-item threshold or the onboarding-seeded `gap_baseline_days` is crossed. Strict three-step order per Section 3.4: acknowledge return with no gap/count reference, one open question (voice or typed), then up to two stalled items surfaced one at a time, `inquiring`-framed. Browser-verified end-to-end.
6. ✅ Task decomposition — `api/decompose.js` + `frontend/web/app.js`: on-demand (not at capture), AI-judgment-only trigger, subtasks are real intents linked via `parent_intent_id`/`subtasks`, editable proposal before approval (round check-icon include/exclude), no forced sequencing, both a global rollup (home feed entry point, grouped by parent + "Also resurfacing") and a full **Task detail** page (`frontend/web/task.html`/`task.js`, new 2026-08-11) grouped by has-a-date/no-date/closed. **Deadline-confirmation prompt (decision #8) is now built** — Task detail asks once per subtask whether a date exists, never all at once. Parent-level "Archive this whole thing" (decision #7: independent of subtask state) also new. Verified locally end-to-end; needs real Mistral key + real-use testing for the decomposition proposal itself.
7. 🟡 Orchestrator MVP (2026-08-13) — `api/orchestrator.js` splits step 4's single call into explicit persona-read + reminder-plan steps per `orchestrator/REVIEW.md`, adds `schedule_hint`/`decomposition_candidate`. `inferDecision()` switched over; `api/infer.js` kept as fallback during the transition.
8. 🟡 Reminder scheduling scaffold (2026-08-13) — `reminder/scheduler.js`: fixed-interval timing + the repair-vs-silent-recovery decision as pure functions, consuming `schedule_hint`. Not wired to a live trigger (no notification channel yet); tone/copy still needs real-user testing, which this scaffold enables but doesn't substitute for.

**Next logical build step:** the two riskiest remaining unknowns both require Ismail's own usage/testing, not more code: (1) does the orchestrator's moment/urgency/framing choice actually feel better calibrated than picking it by hand, now that it's live end-to-end through the two-step pipeline; (2) real reactions to reminder timing/tone — `reminder/scheduler.js`'s intervals and the repair-vs-switch threshold are documented as untuned guesses, and `OPEN_QUESTIONS.md` items 8-10 stay "open"/"partial" until that happens. Also still open: the onboarding "Edit" flow in Settings restarts the ten-question flow from scratch rather than resuming with prior answers pre-filled; `capture/web/`'s service worker cache-first strategy needs a version bump reminder written down somewhere so stale-asset debugging (hit once this session) doesn't recur silently for the next person testing it locally; and the privacy opt-out tiers designed in `PRIVACY.md` §2 need a schema decision from the Data & Schema Agent before they can be implemented.
...
4. *(Answered 2026-08-11 — built into onboarding as Q7: "How long do you usually go before checking in on your own list?" with four answer bands mapped to `gap_baseline_days`.)* Does that mapping (most days→1, every few days→4, couple weeks→14, "I don't keep one"→10 as a cautious default) actually feel right once Recovery Mode fires on real usage, or does it fire too eagerly/too rarely?
...
**Schema:** implemented additively in `schema/intent.json` 0.3.0 as `parent_intent_id`, `subtasks`, `deadline`, `deadline_confirmed_absent` — see `schema/SCHEMA.md`'s migration note. **Build (2026-08-11, complete):** propose/edit/approve flow (`api/decompose.js`, `frontend/web/app.js`, round check-icon toggle per the refined mockup), a global rollup entry point on the home feed, and a full Task detail page (`frontend/web/task.html`/`task.js`) grouped by has-a-date/no-date/closed with the deadline-confirmation prompt (#8) built in and a parent-level archive action. All verified locally end-to-end against the mockup set in `frontend/mockups/`.
