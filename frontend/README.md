...
**2026-08-14 — all four framing modes now render.** `activation-only` and `silent-recovery` (roadmap §3.3) were promised at onboarding Q8 but silently approximated with `direct`/`inquiring` by both API prompts — a preference the setup flow collects and the check-in then ignores. `framingCopy()` in `app.js` now implements both: `activation-only` shows only a model-supplied `activation_step` (never a client-side guess at the task's first step — a confidently wrong instruction is worse than none, so it degrades to `direct` copy if the field is missing or reads like a restated task); `silent-recovery` actually folds the item into Recovery Mode (`foldIntoRecovery()` sets `flagged_for_recovery`, the state `recovery.js` already reads) rather than showing nothing, with an explicit "Let's look now" escape hatch back to `inquiring` for anyone who taps through anyway. `api/orchestrator.js` and `api/infer.js` both return all four framings now; `typical_intent_class` and `stated_goal` (asked at onboarding, previously dead weight) are wired into `orchestrator.js`'s persona-guidance rules — they were reaching the request body already, the endpoint was dropping them on read.

The `?dev=1` manual framing picker (`index.html`) still only offers `direct`/`inquiring` radios — left as a known gap, not a functional one.

**2026-08-14 — installable as a PWA.** `frontend/web/manifest.json` + `sw.js` + `pwa.js` (new) make the actual app shell installable — previously only `capture/web/` was, so "install" got you the mic screen alone. `sw.js` is deliberately network-first with a cache fallback, not cache-first like `capture/web/sw.js` (whose cache-first strategy is documented in `CLAUDE.md` as able to serve stale code after a deploy) — six pages that change often made that risk worse here. `/api/*` is never cached. Real icons live in `/icons/` (192/512/maskable PNG + SVG, replacing `capture/web/icon.svg`'s placeholder), shared by both manifests. `landing/` is a new marketing page now served at `/` (`vercel.json`); the app itself still lives at `/frontend/web/`.
