...
6. 🟡 in progress - **Task decomposition** (Notion roadmap, decomposition decisions, 2026-08-10 follow-up). MVP scaffolded: `api/decompose.js` (Vercel serverless function) + `frontend/web/app.js`'s decompose/propose/rollup flow. AI-judgment-only trigger for v1 (no manual "break this down" button, per Ismail's decision), manual edit step before anything commits. Verified locally end-to-end (headless Chromium, mocked endpoints); needs real Mistral key + real-use testing next. **Known gap (2026-08-10):** mockups 12-13 (`frontend/mockups/`) show a *global* rollup check-in screen (groups subtask check-ins by parent + other resurfacing items across the whole feed in one pass, single "that's enough for now" exit) - what's built is scoped to one parent at a time. Not yet reconciled; paused here per Ismail, pick up later.
